...
namespace Gamestore.DataAccess.IntegrationTests
{
    public class GameRepositoryTests
    {
        ...
        public async Task GetGameWithJoinsAsync_ReturnGameWithPlatformsAndGenres()
        {
            ...
            var game = CreateGame();
            game.GamePlatforms = platformIds.Select(id => new GamePlatform { PlatformId = id }).ToList();
            game.GameGenres = genreIds.Select(id => new GameGenre { GenreId = id }).ToList();
        ...
        private async Task SeedGame(GamestoreDbContext context, Game game)
        {
            context.Games.Add(game);
            await context.SaveChangesAsync();
        }
